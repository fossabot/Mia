...
//    }
    ...
    public init(items: [MediumMenuItem], forViewController: UIViewController) {
        ...
        self.init()
        self.items = items
        height = Device.Size.screenHeight - 80 // auto-calculate initial height based on screen size
        frame = CGRect(x: 0, y: 0, width: Device.Size.screenWidth, height: height)
    ...
    open override func layoutSubviews() {
        ...
        frame = CGRect(x: 0, y: 0, width: Device.Size.screenWidth, height: height);
        contentController?.view.frame = CGRect(x: 0, y: 0, width: Device.Size.screenWidth, height: Device.Size.screenHeight);
        menuContentTableView = UITableView(frame: frame)
    }
    ...
    open func panned(on pan: UIPanGestureRecognizer) {
        ...
        if pan.state == .began || pan.state == .changed {
            ...
            if viewCenter.y >= Device.Size.screenHeight / 2
               && viewCenter.y <= (Device.Size.screenHeight / 2 + height) - bounceOffset {
    ...
    fileprivate func open(animated: Bool = true, completion: (() -> Void)? = nil) {
        ...
        if animated {
            UIView.animate(withDuration: animationDuration, animations: {
                self.contentController?.view.center = CGPoint(x: x, y: Device.Size.screenHeight / 2 + self.height)
            }, completion: { _ in
                UIView.animate(withDuration: self.animationDuration, animations: {
                    self.contentController?.view.center = CGPoint(x: x, y: Device.Size.screenHeight / 2 + self.height - self.bounceOffset)
                }, completion: { _ in
            ...
            contentController?.view.center = CGPoint(x: x, y: Device.Size.screenHeight / 2 + height)
            currentState = .shown
            completion?()
        }
    }
    ...
    fileprivate func close(animated: Bool = true, completion: (() -> Void)? = nil) {
        ...
        if animated {
            UIView.animate(withDuration: animationDuration, animations: {
                ...
                UIView.animate(withDuration: self.animationDuration, animations: {
                    self.contentController?.view.center = CGPoint(x: center.x, y: Device.Size.screenHeight / 2)
                }, completion: { _ in
            ...
            contentController?.view.center = CGPoint(x: center.x, y: Device.Size.screenHeight / 2)
            currentState = .closed
            completion?()
        }
    }
    ...
    fileprivate func openMenuFromCenter(with velocity: CGFloat) {
        ...
        let viewCenterY = Device.Size.screenHeight / 2 + height - bounceOffset
        currentState = .displaying
    ...
    fileprivate func closeMenuFromCenter(with velocity: CGFloat) {
        ...
        let viewCenterY = Device.Size.screenHeight / 2
        currentState = .displaying
