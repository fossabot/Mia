...
public extension Container where Host: NSObject {
    ...
    func unobserve(object: NSObject, keyPath: String? = nil) {
        ...
        let predicate: (KeyPathTarget.Item) -> Bool = { item in
            return item.object === object && (keyPath != nil) ? (keyPath! == item.keyPath) : true
        }
...
class KeyPathTarget: NSObject {
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        ...
        guard let object = object as? NSObject, let keyPath = keyPath, let value = change?[.newKey] else {
            return
        }

        let predicate: (KeyPathTarget.Item) -> Bool = { item in
            return item.object === object && keyPath == item.keyPath
        }
