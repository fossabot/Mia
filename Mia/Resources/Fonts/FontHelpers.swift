...
public extension Device.Screen {
    ...
    public static func size<T>(phone: T, pad: T) -> T {
        ...
        return (Device.Device.isPad ? pad : phone)
    }
    ...
    public static func size(sizes: [ScreenSize: CGFloat]) -> CGFloat {
        
        let screen = Device.Screen.size
        var nearestValue: CGFloat?
        var distance = CGFloat.greatestFiniteMagnitude
...
//                let font = UIFont(name: fontDescription.fontName, size: Scale.fontSize(size: fontDescription.fontSize))
//                else {
