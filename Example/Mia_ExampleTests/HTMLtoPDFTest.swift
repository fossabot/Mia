...
class HTMLtoPDFTest: XCTestCase {
    ...
    func testBlock() -> Void {
        ...
        html = HTMLtoPDF.save(html: htmlstring) { (data, error) in
            ...
            if let data = data {
                ...
                mc.setSubject("HTML To PDF Demo")
                mc.setMessageBody("Please take a look at the attached document", isHTML: false)
                mc.addAttachmentData(data, mimeType: "application/pdf", fileName: "Expense Queue.pdf")
                getTopMostController()?.present(mc, animated: true, completion: nil)
    ...
    func testDelegate() -> Void {

        html = HTMLtoPDF()
        html?.delegate = self

        html?.save(html: htmlstring)
...
extension HTMLtoPDFTest: HTMLtoPDFDelegate {

    func htmlPdfKit(htmlPdfKit: HTMLtoPDF, didSavePdfData data: Data) {
...
extension HTMLtoPDFTest: MFMailComposeViewControllerDelegate {
